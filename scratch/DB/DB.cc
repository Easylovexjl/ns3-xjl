...
//	cmd.AddValue ("logFile", "Log file", logFile);
	...
	cmd.AddValue ("mod", "0=olsr 1=db(DEFAULT) 2=aodv 3=dsdv 4=dsr", mod);
...
void VanetSim::ConfigApp()
{
	...
	else
	{
	  ...
	  for (uint32_t i = 0; i<nodeNum; ++i)
	    {
	      db.SetNodeTypeMap (m_nodes.Get (i), db::CAR);
	    }
	  db.SetNodeTypeMap (m_nodes.Get (nodeNum), db::LOCAL_CONTROLLER);
          db.SetNodeTypeMap (m_nodes.Get (nodeNum+3), db::LOCAL_CONTROLLER);
	  db.ExcludeInterface (m_nodes.Get (nodeNum), 0);
	  db.SetNodeTypeMap (m_nodes.Get (nodeNum+1), db::CAR);//Treat Source and Sink as CAR
	  db.SetNodeTypeMap (m_nodes.Get (nodeNum+2), db::CAR);
	  db.SetRLnSR (range1, range2);
		...
		for (uint32_t i = 0;i<m_nodes.GetN ();++i)
		  {
		    ...
		    Ptr<db::RoutingProtocol> routing =
		        m_nodes.Get (i)->GetObject<db::RoutingProtocol> ();
